---
title: 基于HCL实现MPLS L3VPN跨域Option A (RR)组网
description: 使用H3C Cloud Lab实现MPLS L3VPN跨域Option A (RR)组网
date: 2025-11-10
slug: mpls_l3vpn_opton_a
image: https://ryan1998.dpdns.org/cover/post/mpls-l3vpn-inter-as-option-a/cover-mpls-l3vpn-option-a.png
categories:
    - Datacom
    - MPLS L3VPN
tags:
...
![组网拓扑图](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/topo.png)
...
- ASBR 之间建立 **eBGP** 传递 IPv4 路由，ASBR 之间**不需要**运行 MPLS 和 LDP 协议 
...
> 互联地址编址规范（仅实验环境）： AS1.AS2.VPN.ASx/24

| A端AS | A端设备   | A端接口        | A端地址           | VPN | Z端地址           | Z端接口        | Z端设备   | Z端AS |
| ---- | ------ | ----------- | -------------- | --- | -------------- | ----------- | ------ | ---- |
| 10   | ASBR_1 | GE0/0/10.11 | 10.20.11.10/24 | 11  | 10.20.11.20/24 | GE0/0/10.11 | ASBR_2 | 20   |
| 10   | ASBR_1 | GE0/0/10.22 | 10.20.22.10/24 | 22  | 10.20.22.20/24 | GE0/0/10.22 | ASBR_2 | 20   |

> VPN实例RD、RT分配规范（仅实验环境）：
> 
> MPLS L3VPN Inter-AS Option A 方式
> 
> - **不要求** `route-target` 必须要匹配
> 
> - 不同的AS内可**自定义** `route-target`
> 
> RD/RT ：**AS:VPN**
...
ip vpn-instance 11
 ...
 vpn-target 10:11 import-extcommunity
 vpn-target 10:11 export-extcommunity
#
 interface GigabitEthernet0/0/10.11
  description TO AS20_ASBR_2_VPN11
  ip binding vpn-instance 11
  ip address 10.20.11.10 24
  vlan-type dot1q vid 11
#
ip vpn-instance 22
 route-distinguisher 10:22
 vpn-target 10:22 import-extcommunity
 vpn-target 10:22 export-extcommunity
#
 interface GigabitEthernet0/0/10.22
  description TO AS20_ASBR_2_VPN22
  ip binding vpn-instance 22
  ip address 10.20.22.10 24
...
ip vpn-instance 11
 ...
 vpn-target 20:11 import-extcommunity
 vpn-target 20:11 export-extcommunity
#
 interface GigabitEthernet0/0/10.11
  description TO AS10_ASBR_1_VPN_11
  ip address 10.20.11.20 24
  vlan-type dot1q vid 11
#
ip vpn-instance 22
 route-distinguisher 20:22
 vpn-target 20:22 import-extcommunity
 vpn-target 20:22 export-extcommunity
#
 interface GigabitEthernet0/0/10.22
  description TO AS10_ASBR_1_VPN_22
  ip address 10.20.22.20 24
  vlan-type dot1q vid 22
#
```
...
> 1. 因为 RR 上不接业务，也不配置 VPN 实例，所以需要在 VPNv4 地址簇下配置
> 
>    **`undo policy vpn-target`**
> 
> 2. 因为 AS 域内的 RR 和 PE 均没有 ASBR 之间互联地址的路由，所以需要在 ASBR 侧的  VPNv4 地址簇下配置
> 
...
bgp 10
 #
 ip vpn-instance 11
  group AS20_VPN11 external
  peer AS20_VPN11 as-number 20
  peer 10.20.11.20 group AS20_VPN11
  #
  address-family ipv4 unicast
   peer AS20_VPN11 enable
 # 
 ip vpn-instance 22
  group AS20_VPN22 external
  peer AS20_VPN22 as-number 20
  peer 10.20.22.20 group AS20_VPN22
  #
  address-family ipv4 unicast
   peer AS20_VPN22 enable
#
```
...
bgp 20
 #
 ip vpn-instance 11
  group AS10_VPN11 external
  peer AS10_VPN11 as-number 10
  peer 10.20.11.10 group AS10_VPN11
  #
  address-family ipv4 unicast
   peer AS10_VPN11 enable
 # 
 ip vpn-instance 22
  group AS10_VPN22 external
  peer AS10_VPN22 as-number 10
  peer 10.20.22.10 group AS10_VPN22
  #
  address-family ipv4 unicast
   peer AS10_VPN22 enable
#
```


> BGP配置完成后，可检查BGP对等体建立情况情况
> 
> `display bgp peer vpnv4`
   ![display bgp peer vpnv4](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/asbr1_display_bgp_peer_vpnv4.png)
>
>
> `display bgp peer ipv4 vpn-instance-all` 
   ![display bgp peer ipv4 vpn-instance-all](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/asbr1_display_bgp_peer_ipv4_vpn-instance-all.png)
...
1. 查看接口地址：`display ip interface brief`
   ![`display ip interface brief`](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/pe1_display_ip_interface_brief.png)


2. 查看ISIS邻居状态：`display isis peer`
   ![`display isis peer`](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/pe1_display_isis_peer.png)


3. 查看LDP邻居状态：`display mpls ldp peer`
   ![display mpls ldp peer](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/pe1_display_mpls_ldp_peer.png)


4. 查看MP-BGP对等体状态：`display bgp peer vpnv4`
   ![display bgp peer vpnv4](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/pe1_display_bgp_peer_vpnv4.png)


5. 查看VPN路由表：`display ip routing-table vpn-instance 11`
   ![display ip routing-table vpn-instance 11](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/pe1_display_ip_routing-table_vpn-instance_11.png)
### CE_1
 
1. 查看接口地址：`display ip interface brief`
   ![display ip interface brief](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/ce1_display_ip_interface_brief.png)


2. 查看 ISIS 邻居状态：`display isis peer`
   ![display isis peer](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/ce1_display_isis_peer.png)


3. 查看 VPN 路由表：`display ip routing-table vpn-instance 11`
   ![display ip routing-table vpn-instance 11](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/ce1_display_ip_routing-table_vpn-instance_11.png)


4. 使用CE1 Loop11和CE3 Loop11进行Ping测试：`ping -a 10.11.1.6 -vpn-instance 11 20.11.1.6`
   ![display ip routing-table vpn-instance 11](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/ce1_ping_-a_10.1.1.6_-vpn-instance_11_20.1.1.6.png)

### CE_2

1. 查看 OSPF 邻居状态：`display ospf peer`
   ![display ospf peer](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/ce2_display_ospf_peer.png)


2. 查看 VPN 路由表：`display ip routing-table vpn-instance 22`
   ![display ospf peer](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/ce2_display_ip_routing-table_vpn-instance_22.png)


3. 使用CE2 Loop22 和 CE4 Loop22 进行 Ping 测试： `ping -a 10.22.2.7 -vpn-instance 22 20.22.2.7`
   ![display ip routing-table vpn-instance 11](https://ryan1998.dpdns.org/illus/mpls-l3vpn-inter-as-option-a/ce2_ping_-a_10.22.2.7_-vpn-instance_22_20.22.2.7.png)`
...
interface GigabitEthernet0/0/10.22
 description TO AS20_ASBR_2_VPN22
 ip binding vpn-instance 22
 ip address 10.20.22.10 255.255.255.0
 vlan-type dot1q vid 22
#
bgp 10
 router-id 10.0.0.2
...
interface GigabitEthernet0/0/10.11
 description TO AS10_ASBR_1_VPN_11
 ip binding vpn-instance 11
 ip address 10.20.11.20 255.255.255.0
 vlan-type dot1q vid 11
#
interface GigabitEthernet0/0/10.22
 description TO AS10_ASBR_1_VPN_22
 ip binding vpn-instance 22
 ip address 10.20.22.20 255.255.255.0
 vlan-type dot1q vid 22
#
bgp 20
 router-id 20.0.0.2
